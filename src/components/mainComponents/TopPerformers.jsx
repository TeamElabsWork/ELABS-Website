...
/* ── CARD ── */
function Card({ name, image, index, big }) {
  ...
  return (
    <div ref={ref} className={big ? "tp-big-card" : "tp-sm-card"}>
      <div className="tp-card-glow" />
      <div className={big ? "tp-big-photo-wrap" : "tp-sm-photo-wrap"}>
      ...
      <p className={big ? "tp-big-name" : "tp-sm-name"}>{name}</p>
    </div>
  );
}
  ...
  return (
    <div className="tp-page">
      ...
      <div className="tp-inner">
        <p className="tp-badge">⚡ ELabs Official Website</p>
        ...
        {/* ── KIITO APP ── */}
        <SectionBlock badge="📱 Initiative" title="KIITO APP" desc="The team behind your ultimate campus companion." link="/initiatives/kiito-app" />
        <div className="tp-grid-6">{kiitoMembers.map((m,i) => <Card key={i} index={i} {...m} big />)}</div>

        {/* ── BLAZE BIT ── */}
        <SectionBlock badge="🔥 Initiative" title="BLAZE BIT" desc="Smart study tools built by passionate engineers." link="/initiatives/blaze-bit" />
        <div className="tp-grid-6">{blazeMembers.map((m,i) => <Card key={i} index={i} {...m} big />)}</div>
        ...
        <div className="tp-divider" />
        <h2 className="tp-section-title" style={{ marginBottom: "24px" }}>LEAD / PROJECT MANAGER</h2>

        <div className="tp-lead-card">
          {/* Full-width rectangular photo */}
          <div className="tp-lead-photo-banner">
            <img src="/Images/members/default.jpg" alt="Abinash Mohanty" className="tp-lead-banner-img"
              onError={(e) => (e.currentTarget.src = "https://placehold.co/1000x400/1a1a1a/ff9a33?text=Abinash+Mohanty")} />
            <div className="tp-lead-banner-overlay" />
          </div>
...
.tp-fuzzy-wrap{margin-bottom:10px;display:flex;justify-content:center;width:100%;max-width:100%;overflow:hidden}

.tp-section-header{display:flex;align-items:center;gap:16px;flex-wrap:wrap;justify-content:center;margin-bottom:8px}
.tp-section-title{font-size:clamp(1.4rem,3.5vw,2.4rem);font-weight:800;letter-spacing:.04em;text-align:center;background:linear-gradient(135deg,#ff9a33,#ff6b00,#ff9a33);background-size:200% auto;-webkit-background-clip:text;background-clip:text;-webkit-text-fill-color:transparent;animation:tpSheen 4s ease-in-out infinite;line-height:1.1}
@keyframes tpSheen{0%,100%{background-position:0% 50%}50%{background-position:100% 50%}}

.tp-view-btn{font-size:.78rem;font-weight:700;color:#000;background:#ff9a33;border:none;padding:6px 18px;border-radius:999px;cursor:pointer;letter-spacing:.04em;transition:background .25s,transform .25s,box-shadow .25s}
...
@media(max-width:760px){.tp-grid-7{grid-template-columns:repeat(4,1fr)}.tp-grid-6{grid-template-columns:repeat(3,1fr)}}
@media(max-width:500px){.tp-grid-7{grid-template-columns:repeat(3,1fr)}.tp-grid-6{grid-template-columns:repeat(2,1fr)}}
...
.tp-sm-card:hover .tp-card-glow,.tp-big-card:hover .tp-card-glow{opacity:1}
...
.tp-sm-card:hover .tp-card-img,.tp-big-card:hover .tp-card-img{transform:scale(1.1)}
...
.tp-sm-card:hover .tp-card-shimmer,.tp-big-card:hover .tp-card-shimmer{background-position:200% 0}
...
.tp-sm-card:hover .tp-sm-name,.tp-big-card:hover .tp-big-name{opacity:1;transform:translateY(0);color:#ffcc88}

/* ── LEAD SECTION ── */
.tp-lead-card{width:100%;max-width:1000px;border:1px solid rgba(255,154,51,.2);border-radius:24px;overflow:hidden;background:rgba(255,154,51,.03);backdrop-filter:blur(12px)}

.tp-lead-photo-banner{position:relative;width:100%;height:340px;overflow:hidden}
.tp-lead-banner-img{width:100%;height:100%;object-fit:cover;object-position:center top;display:block;transition:transform .5s}
.tp-lead-card:hover .tp-lead-banner-img{transform:scale(1.03)}
.tp-lead-banner-overlay{position:absolute;inset:0;background:linear-gradient(to top,rgba(0,0,0,.85) 0%,transparent 60%)}

.tp-lead-body{padding:32px 36px 36px}
@media(max-width:640px){.tp-lead-body{padding:24px 20px 28px}.tp-lead-photo-banner{height:220px}}
...
.tp-init-row{display:flex;gap:24px;flex-wrap:wrap}
@media(max-width:768px){.tp-init-row{justify-content:center}}
...
.tp-init-circle span{font-size:.65rem;font-weight:600;color:rgba(255,220,170,.85);letter-spacing:.04em}
      `}</style>
    </div>
  );
}
